...
def index():
    ...
    roasts.sort(
        key=lambda r: r.get("roast_start_time") or r.get("roast_date") or datetime.min,
        reverse=True,
    )
        ...
        if roast.get("key_timings") and roast.get("total_duration_seconds"):
            ...
            for timing in roast["key_timings"]:
                if "First Crack Start" in timing["event_name"]:
                    fc_start = timing[
                        "time_seconds"
                    ]  # Will keep updating to latest one
            if fc_start:
...
@app.route("/api/label/preferences", methods=["GET"])
def api_label_preferences():
    """Return template/font/aspect defaults derived from the most recently
    updated bean that has a saved label. Used as the default when opening the
    label modal for a bean with no label of its own."""
...
def api_roast_start(roast_id):
    ...
    get_roasts_collection().update_one(
        {"_id": ObjectId(roast_id)}, {"$set": update_data}
    )
...
def api_roast_add_timing(roast_id):
    ...
    timing_event = {"event_name": data["event_name"], "time_seconds": time_seconds}
...
def api_roast_add_review(roast_id):
    ...
    get_roasts_collection().update_one(
        {"_id": ObjectId(roast_id)},
        {"$push": {"reviews": review}, "$set": {"updated_at": current_time_utc}},
    )
...
def calculate_ror(roast_id, current_temp, current_time):
    ...
    for reading in history:
        ...
        if (
            time_diff_from_target <= ROR_TOLERANCE_SECONDS
            and time_diff_from_target < best_diff
        ):
...
def api_get_db_settings():
    ...
    return jsonify({"mode": get_current_db_mode(), "default": DEFAULT_DB})
