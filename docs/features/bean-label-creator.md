...
Each label value has its own Font size (%) control, scaling that field from 50–200%, including line spacing and name wrapping. Blank uses each template’s default (100%). Save Label Info stores explicitly entered field sizes in the bean’s label; clearing it and saving removes the override. Reopening restores the saved field sizes. It is per bean and does not seed other beans. The preview and both PNG exports use the same sized canvas. Large text can exceed the available layout; use the preview to choose a suitable size.
